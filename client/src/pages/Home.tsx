        ...
        <div className="relative z-10 max-w-[1200px] mx-auto px-6 md:px-12">
          <div className="text-center mb-12">
            <p className="text-xs font-mono text-[#E8341A] tracking-[0.25em] uppercase mb-4">
              {lang === "zh" ? "使用场景 · HOW IT WORKS" : "HOW IT WORKS · 使用场景"}
            </p>
            <h2 className="font-display text-4xl md:text-5xl font-black leading-tight mb-4">
              {lang === "zh" ? (
                <>{"一句话，"}<br /><span className="text-[#E8341A]">{"AI 帮你搞定。"}</span></>
              ) : (
                <>{"Just Say It,"}<br /><span className="text-[#E8341A]">{"AI Does the Rest."}</span></>
              )}
            </h2>
            <p className="text-white/40 text-base max-w-xl mx-auto">
              {lang === "zh"
                ? "插入 SD 卡，对着 ClawPool 说一句话——照片自动存入、分类、打标签。"
                : "Insert an SD card, speak to ClawPool — photos are automatically stored, sorted, and tagged."}
            </p>
          </div>
          ...
          <div className="relative rounded-sm overflow-hidden border border-white/10 shadow-2xl"
            ...
            <div className="relative w-full" style={{ paddingBottom: "56.25%" }}>
              {[
                "https://d2xsxph8kpxj0f.cloudfront.net/310519663430257498/jfDsKVzTVmzoN4bJ42z8Cq/storyboard_01_sd_insert-4GHtBU2Lomkzkix5UtLuR3.webp",
                ...
                "https://d2xsxph8kpxj0f.cloudfront.net/310519663430257498/jfDsKVzTVmzoN4bJ42z8Cq/storyboard_04_ai_response-gGQZcuH5TmhtMRcPDZZEL9.webp",
                "https://d2xsxph8kpxj0f.cloudfront.net/310519663430257498/jfDsKVzTVmzoN4bJ42z8Cq/storyboard_05_result-Z9XvYXrHUdvYwNsw4MvyvU.webp",
              ].map((src, i) => (
                <img
                  key={i}
                  src={src}
                  alt={`Storyboard step ${i + 1}`}
                  className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-700 ${storyStep === i ? "opacity-100" : "opacity-0"}`}
                />
              ))}
              ...
              {/* Step badge top-left */}
              <div className="absolute top-6 left-6 flex items-center gap-2 bg-black/60 backdrop-blur-sm border border-white/10 px-3 py-1.5 rounded-sm">
              ...
              <button
                onClick={() => setStoryStep((s) => (s - 1 + 5) % 5)}
                className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/40 backdrop-blur-sm border border-white/10 flex items-center justify-center text-white/60 hover:text-white hover:bg-black/60 transition-all"
                aria-label="Previous step"
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M15 18l-6-6 6-6"/></svg>
              </button>
              <button
                onClick={() => setStoryStep((s) => (s + 1) % 5)}
                className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/40 backdrop-blur-sm border border-white/10 flex items-center justify-center text-white/60 hover:text-white hover:bg-black/60 transition-all"
          ...
          <div className="flex flex-wrap justify-center gap-3 mt-8">
            {(lang === "zh"
              ? ["① 插入 SD 卡", "② 语音指令", "③ AI 处理中", "④ AI 回应", "⑤ 完成"]
              : ["① Insert SD", "② Voice Command", "③ AI Processing", "④ AI Response", "⑤ Done"]
            ).map((label, i) => (
              <button
                key={label}
                onClick={() => setStoryStep(i)}
                className={`px-4 py-1.5 border rounded-sm text-xs font-mono tracking-wide transition-all ${
                  storyStep === i
                    ? "bg-[#E8341A]/10 border-[#E8341A]/50 text-[#E8341A]"
